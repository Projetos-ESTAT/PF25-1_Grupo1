...
# Referencial Teórico
  ...
  A análise de dados é um processo de investigar, processar, limpar e modelar dados a fim de extrair informações, observar padrões e realizar inferências a partir de técnicas estatísticas. Esta análise utilizou de tais técnicas a fim de atingir o objetivo do estudo. 
  
  Desse modo, a análise descritiva foi uma abordagem usada para resumir, organizar e apresentar de forma resumida os dados. Neste estudo, ela foi utilizada para calcular a média, mediana, variância, desvio padrão e quartis. Além disso, para a visualização gráfica, foram constrúidos gráficos de dispersão - 

# Metodologia

  Este relatório é constituído por métodos estatíticos que serão descritos a seguir de acordo com o que foi utilizado.
  
 • **Leitura dos dados**: Os dados foram importados e filtrados a partir de uma planilha em formato CSV.
 • **Análise Descritiva**: Foram calculadas medidas de tendência central (média e mediana), dispersão (desvio padrão e variância) e posição (quartis).
 • **Visualização Gráfica**: Construiu-se gráficos de dispersão e de linhas, além de boxplots.
 • **Análise de Regressão Linear**: Aplicada para modelar a relação entre as variáveis.

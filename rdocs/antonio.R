...
analise_3[1:12,1] <- "F.H.C." 
analise_3[13:60,1] <- "Lula 1"
analise_3[61:108,1] <- "Lula 2"
analise_3[109:156,1] <- "Dilma"
analise_3[157:204,1] <- "Temer"
analise_3[205:252,1] <- "Bolsonaro"

#Separando os mandatos para a análise para a tabela
FHC_Medidas <- filter(analise_3, ano == "F.H.C.")
Lula1_Medidas <- filter(analise_3, ano == "Lula 1")
Lula2_Medidas <- filter(analise_3, ano == "Lula 2")
Dilma_Medidas <- filter(analise_3, ano == "Dilma")
Temer_Medidas <- filter(analise_3, ano == "Temer")
...
print_quadro_resumo(analise_3$salario_minimo)

analise_3[,2] <- as.factor(analise_3[,2])
describe(Lula_Medidas1$salario_minimo)
sd(Lula_Medidas1$salario_minimo)
quantile(Lula_Medidas1$salario_minimo, 0.25)
quantile(Lula_Medidas1$salario_minimo, 0.75)

describe(Lula_Medidas2$salario_minimo)
sd(Lula_Medidas2$salario_minimo)
quantile(Lula_Medidas2$salario_minimo, 0.25)
quantile(Lula_Medidas2$salario_minimo, 0.75)
...
analise3_graf <- ggplot(analise_3) +
